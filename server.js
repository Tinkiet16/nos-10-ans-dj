...
app.get("/api/playlist-en-cours", async (req, res) => {
  ...
  const t = await spotify(`/playlists/${id}/tracks?fields=items(track(name,uri,artists(name),album(images)))&limit=100`);
  const data = {
    actif: true,
    name: info.body?.name || "Playlist",
    tracks: (t.body?.items || []).filter(i => i.track).map(i => mapTrack(i.track)),
  };
  playlistEnCoursCache = { id, at: Date.now(), data };
  res.json(data);
});
...
app.get("/api/gplaylists/:id/tracks", async (req, res) => {
  ...
  const r = await spotify(`/playlists/${p.id}/tracks?fields=items(track(name,uri,artists(name),album(images)))&limit=50`);
  const tracks = (r.body?.items || []).filter(i => i.track).map(i => mapTrack(i.track));
